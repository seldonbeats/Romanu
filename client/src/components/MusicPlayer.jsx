...
const addToCartHandler = () => {
    const newItem = {
      id: allSongs[song]?._id,
      name: allSongs[song]?.name,
      price: allSongs[song]?.price,
      image: allSongs[song]?.imageURL,
    };

    const findItem = cart.find((item) => item.id === allSongs[song]?._id);
    ...
    <div className="w-full h-full flex items-center  overflow-hidden">
      ...
      >
        ...
        <div className="flex items-start flex-col">
          <NavLink to={`songdetails/${allSongs[song]?._id}`}>
            <p className="text-md mt-1 text-headingColor font-semibold  hover:text-red-500">
            {allSongs[song]?.artist}{" "}
             
              <span className="text-base">- {`${allSongs[song]?.name.length > 20
          ...
          {allSongs[song]?.BPM}BPM,{allSongs[song]?.language}
          </p>
        ...
        <motion.i
          ...
          >
            <RiPlayListFill className="text-textColor hover:text-headingColor text-3xl cursor-pointer mr-4" />
          </motion.i>
          <button
          id={allSongs[song]?._id}
          type="button"
          ...
          <input
            type="hidden"
            value={allSongs[song]?.name}
            id={`hiddenname${allSongs[song]?._id}`}
          />
          <input
            type="hidden"
            value={allSongs[song]?.price}
            id={`hiddenprice${allSongs[song]?._id}`}
          />
          <input
            type="hidden"
            value={allSongs[song]?.imageURL}
            id={`hiddenimage${allSongs[song]?._id}`}
          />
        </div>
      ...
      {/* <>
        <Transition appear show={isOpen} as={Fragment}>
          <Dialog as="div" className="relative" open={isOpen} onClose={() => setIsOpen(false)}>
            <Transition.Child
              as={Fragment}
              ...
              <div className="flex min-h-full items-center justify-center p-4 text-center z-50">
                ...
                >
                  <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all z-50">
                    ...
                    ><div className="flex ml-auto pl-8 justify-end cursor-pointer text-xl z-50" onClick={() => setIsOpen(false)}><h1>X</h1></div>
                      Payment successful
                    </Dialog.Title>
